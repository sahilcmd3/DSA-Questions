...
Designing Solution:
After a little bit of thinking, it may click that we can map our keys not to the values directly, but to the linked list nodes that represent
these values. Mapping keys to list nodes means that the dictionary will hold node references that don't depend on node positions in
the list itself. So we would be able to rearrange list items without a need to remap them in the dictionary.

                    LRU CACHE

      Hashtable                   Linked List
        key 1       -->         key 1   --  value 1     --> Most recently used
                                  |           |
        key 2       -->         key 2   --  value 2
                                  |           |
        key 3       -->         key 3   --  value 3
                                  |           |
        key n-1     -->         key n-1 --  value n-1
                                  |           |
        key n       -->         key n   --  value n     --> Least recently used
"""
